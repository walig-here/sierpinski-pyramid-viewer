
from renderer import Renderer
from Shapes import ColorRgb, Triangle, Vertex
from eventhandler import EventHandler
import pygame
from pygame.locals import *

class Application:
    def __init__(self, name, scr_w, scr_h):
        pygame.init()
        pygame.display.set_caption(name)

        self.__frame = pygame.display.set_mode((scr_w, scr_h), DOUBLEBUF|OPENGL)
        self.__renderer = Renderer(self.__frame, ColorRgb(100, 100, 100))

        self.__event_handler = EventHandler(self, self.__renderer)
        self.__clock = pygame.time.Clock()

    def shutdown(self):
        pygame.quit()
        quit(0)

    def run(self):
        while True:
            # Obsługa zdarzeń
            self.__event_handler.handleEvents()

            triangle = Triangle([
                Vertex(ColorRgb(50, 168, 82), 0.5, 0.5, 0),
                Vertex(ColorRgb(235, 64, 52), -0.5, 0.5),
                Vertex(ColorRgb(50, 168, 82), 0.0, -0.5)
            ])
            self.__renderer.shapes_to_render.append(triangle)

            # Renderowanie
            self.__renderer.render(1)

            # limit do 60 fps
            # src: https://www.pygame.org/docs/
            self.__clock.tick(60)
